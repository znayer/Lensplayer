...
public class NetUtils {
	...
	public static InetAddress getDeviceSubnetMask(Context ctx) {
		WifiManager wifii = (WifiManager) ctx
		...
		if (d.netmask == 0) {
			...
			try {
				NetworkInterface networkInterface = NetworkInterface
						.getByInetAddress(NetUtils.intToInet(d.ipAddress));
				//get(1) e` IP4
				byte[] subDario = subnetLenghtToSubnetAddressDario(networkInterface
						.getInterfaceAddresses().get(1)
						.getNetworkPrefixLength());
				Log.w("tag",
						"Emergency subnet recovery from IP Address mask lenght:"
								+ networkInterface.getInterfaceAddresses()
								.get(1).getNetworkPrefixLength());
	...
	public static String getDeviceGatewayString(Context ctx) {
		byte[] bytes = BigInteger.valueOf(getDeviceGateway(ctx))
				.toByteArray();
		NetUtils.reverse(bytes);
		InetAddress address;
		try {
	 ...
	 * subnet mask non ha almeno un byte diverso da zero, l'indirizzo IP non pu
	 * essere utilizzato.
	 *
	 * @param toverify
	 */
	public static boolean belongsToNode(InetAddress toverify,
		...
		Log.d("tag",
				"testing belongsToNode subnetMask[]:"
						+ subnetMask.getHostName() + " toverify[]:"
						+ toverify.getHostName());
	...
	private static byte[] subnetLenghtToSubnetAddressDario(int subnet) {
		...
		for (int i = 0; i < 8; i++) {
			if (i >= low_bit)
				submask[3] += Math.pow(2, i);
			if (i + 8 >= low_bit)
				submask[2] += Math.pow(2, i);
			if (i + 16 >= low_bit)
				submask[1] += Math.pow(2, i);
			if (i + 24 >= low_bit)
				submask[0] += Math.pow(2, i);
		}
		return submask;
	}
	 ...
	 * subnetmask il risultato ottenuto  uguale alla and bit a bit del proprio
	 * indirizzo IP con la subnet mask.
	 *
	 * @param toverify
	 */
	public static boolean belongsToSameSubnet(InetAddress toverify,
											  InetAddress subnetMask, InetAddress localH) {
		// int sub = Constants.getSubnet(context);

		// byte[] bytes = BigInteger.valueOf(sub).toByteArray();
		// InetAddress subnet = intToInet(sub);
		Log.d("tag",
				"testing subnet[]:" + subnetMask.getHostName()
						+ " toverify[]:" + toverify.getHostName()
						+ " localH[]:" + localH.getHostName());
		byte[] subnetAddr = subnetMask.getAddress();
		byte[] actual = toverify.getAddress();
	...
	public static InetAddress extractTargetAddress(ArrayList<Short> mac)
	...
	public static String openHTMLStringfromURI(Context context, String id)
			throws FileNotFoundException {
		ContentResolver cr = context.getContentResolver();
		Log.d("tag", "fileUriString = " + id);
...
/*
	public static JSONObject getJSONSoulissDevice(
			SoulissTypical soulissTypical) {
		JSONObject objecttyp = new JSONObject();
		try {
			objecttyp.put("typ", Integer.toHexString(soulissTypical
					.getTypicalDTO().getTypical()));
			...
			if (soulissTypical.getClass() == SoulissTypical5nCurrentVoltagePowerSensor.class) {
				...
				objecttyp
						.put("val",
								((SoulissTypical5nCurrentVoltagePowerSensor) soulissTypical)
										.getOutputFloat());
			} else {
	...
	public static JSONObject getJSONSoulissLiveData(
			SoulissTypical soulissTypical) {
		JSONObject objecttyp = new JSONObject();
		try {
			objecttyp.put("typ", Integer.toHexString(soulissTypical
