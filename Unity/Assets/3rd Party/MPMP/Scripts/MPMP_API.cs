...
namespace monoflow
{
    public partial class MPMP : MonoBehaviour
    {
        ...
        {
            return MPMP_GetBufferLevel(_id);
        }
        ...
        public void Load()
        {
           ...
           // if (_loadLock) return;
            ...
            //For Media Foundation we need to reverse the url encoding to support fancy unicode characters(umlaute,chinese..)
...
*/
            ...
            set
            {
                ...
                if (isChanged)
                {
                   ...
                   // SetAutoPlay(_id, _autoPlay);
                    if (_hasNativeAccess)
                    {
                        MPMP_SetAutoPlay(_id, _autoPlay);
                    }
                }
            }
        }
...
#if SEEK_TOLERANCE
							MPMP_SeekToWithTolerance(_id,_seek,0f,normalized);
#else
							MPMP_SeekTo(_id, _seek,normalized);
#endif
#else
                //Debug.Log("SeekTo:" + _seek);
        ...
        {
            _volume = (float)MPMP_GetCurrentVolume(_id);
        }
        ...
        public float rate
        {
            ...
            set
            {
                ...
                if (isChanged)
                {
                    ...
                    if (_hasNativeAccess)
                    {
                        MPMP_SetPlaybackRate(_id, _rate);
                    }
                }
            }
        }
        ...
        {
            _rate = MPMP_GetPlaybackRate(_id);
        }
        ...
        private void _UpdateCurrentPosition()
        {
            ...
            if (_hasNativeAccess)
            _currentPosition = (double)MPMP_GetCurrentPosition(_id);
        ...
        private void _UpdateDuration()
        {
            //Internal call to plugin
            if (_hasNativeAccess)
            _duration = (double)MPMP_GetDuration(_id);
        }
...
#if !(UNITY_WEBGL && !UNITY_EDITOR)
            if (_hasNativeAccess)
                MPMP_GetNativeVideoSize(_id, ref _currentNativeVideoSize);
#else
           // Debug.Log("W:"+GetNativeVideoSizeW(_id));
           // Debug.Log("H:"+GetNativeVideoSizeH(_id));
           _currentNativeVideoSize = new Vector2((float)MPMP_GetNativeVideoSizeW(_id), (float)MPMP_GetNativeVideoSizeH(_id));
#endif
        }
        ...
        {
            MPMP_SetAudioTrack(_id, (int)Mathf.Clamp(index, 0.0f, 8.0f)); 
        }
        ...
        {
            return MPMP_HasAudioTrack(_id, (int)Mathf.Clamp(index, 0.0f, 8.0f));
        }
