...
// 
// StatsMonitorWidget.cs - version 1.0.0 - Updated 01-02-2016 22:08
//  
...
namespace StatsMonitor
{
	...
	public class StatsMonitorWidget : MonoBehaviour
	{
		...
		/* General parameters */
		[SerializeField] private SMMode _mode = SMMode.Active;
		[SerializeField] private SMRenderMode _renderMode = SMRenderMode.Overlay;
		[SerializeField] private SMStyle _style = SMStyle.Standard;
		[SerializeField] private SMTheme _theme = SMTheme.Default;
		[SerializeField] private SMAlignment _alignment = SMAlignment.UpperRight;
		...
		[SerializeField] internal bool autoScale = false;
		[SerializeField] internal Color colorBGUpper = SMUtil.HexToColor32("00314ABE");
		[SerializeField] internal Color colorBGLower = SMUtil.HexToColor32("002525C8");
		[SerializeField] internal Color colorGraphBG = SMUtil.HexToColor32("00800010");
		[SerializeField] internal Color colorFPS = Color.white;
		[SerializeField] internal Color colorFPSWarning = SMUtil.HexToColor32("FFA000FF");
		[SerializeField] internal Color colorFPSCritical = SMUtil.HexToColor32("FF0000FF");
		[SerializeField] internal Color colorFPSMin = SMUtil.HexToColor32("999999FF");
		[SerializeField] internal Color colorFPSMax = SMUtil.HexToColor32("CCCCCCFF");
		[SerializeField] internal Color colorFPSAvg = SMUtil.HexToColor32("00C8DCFF");
		[SerializeField] internal Color colorFXD = SMUtil.HexToColor32("C68D00FF");
		[SerializeField] internal Color colorMS = SMUtil.HexToColor32("C8C820FF");
		[SerializeField] internal Color colorObjCount = SMUtil.HexToColor32("00B270FF");
		[SerializeField] internal Color colorGCBlip = SMUtil.HexToColor32("00FF00FF");
		[SerializeField] internal Color colorMemTotal = SMUtil.HexToColor32("4080FFFF");
		[SerializeField] internal Color colorMemAlloc = SMUtil.HexToColor32("B480FFFF");
		[SerializeField] internal Color colorMemMono = SMUtil.HexToColor32("FF66D1FF");
		[SerializeField] internal Color colorSysInfoOdd = SMUtil.HexToColor32("D2EBFFFF");
		[SerializeField] internal Color colorSysInfoEven = SMUtil.HexToColor32("A5D6FFFF");
		[SerializeField] internal Color colorOutline = SMUtil.HexToColor32("00000000");
...
#if UNITY_EDITOR
		[HideInInspector] [SerializeField] internal bool hotKeyGroupToggle = false;
		[HideInInspector] [SerializeField] internal bool touchControlGroupToggle = false;
		...
		/// <summary>
		///		The theme of Stats Monitor. Allows to select several default themes that
			...
			set
			{
				...
				switch (_theme)
				{
					case SMTheme.Default:
						colorBGUpper.a = SMUtil.Normalize(190.0f);
						colorBGLower.a = SMUtil.Normalize(200.0f);
						colorGraphBG.a = SMUtil.Normalize(16.0f);
						colorOutline.a = SMUtil.Normalize(0.0f);
						break;
					case SMTheme.Plain:
						colorBGUpper.a = SMUtil.Normalize(0.0f);
						colorBGLower.a = SMUtil.Normalize(0.0f);
						colorGraphBG.a = SMUtil.Normalize(0.0f);
						colorOutline.a = SMUtil.Normalize(0.0f);
						break;
					case SMTheme.Opaque:
						colorBGUpper.a = SMUtil.Normalize(255.0f);
						colorBGLower.a = SMUtil.Normalize(255.0f);
						colorGraphBG.a = SMUtil.Normalize(255.0f);
						colorOutline.a = SMUtil.Normalize(0.0f);
						break;
					case SMTheme.Oblique:
						...
						colorBGLower.a = SMUtil.Normalize(0.0f);
						colorGraphBG.a = SMUtil.Normalize(0.0f);
						colorOutline.a = SMUtil.Normalize(190.0f);
						break;
				}
		...
		public int Scale
		{
			...
			set
			{
				...
				scale = value;
				if (!enabled || _mode != SMMode.Active) return;
		...
		public bool ThrottleFrameRate
		{
			...
			set
			{
				...
				if (!enabled || _mode == SMMode.Inactive) return;
		...
		public void Toggle()
		{
			if (_mode == SMMode.Inactive) Mode = SMMode.Active;
			else if (_mode == SMMode.Active) Mode = SMMode.Inactive;
		}
		...
		{
			if (_style == SMStyle.Minimal) Style = SMStyle.StatsOnly;
			else if (_style == SMStyle.StatsOnly) Style = SMStyle.Standard;
			else if (_style == SMStyle.Standard) Style = SMStyle.Full;
			else if (_style == SMStyle.Full) Style = SMStyle.Minimal;
		}
		...
		{
			if (_alignment == SMAlignment.UpperLeft) Align(SMAlignment.UpperCenter);
			else if (_alignment == SMAlignment.UpperCenter) Align(SMAlignment.UpperRight);
			else if (_alignment == SMAlignment.UpperRight) Align(SMAlignment.LowerRight);
			else if (_alignment == SMAlignment.LowerRight) Align(SMAlignment.LowerCenter);
			else if (_alignment == SMAlignment.LowerCenter) Align(SMAlignment.LowerLeft);
			else if (_alignment == SMAlignment.LowerLeft) Align(SMAlignment.UpperLeft);
		}
		...
		internal void Invalidate(SMViewInvalidationType type, SMInvalidationFlag flag = SMInvalidationFlag.Any, bool invalidateChildren = true)
		{
			...
			if (_style != SMStyle.Minimal && (type == SMViewInvalidationType.All || (type == SMViewInvalidationType.Style && flag == SMInvalidationFlag.Background)))
			{
				CreateBackground();
			}
            ...
            if (totalWidth > 0.0f || totalHeight > 0.0f)
            {
                ...
                SMUtil.RTransform(gameObject, Vector2.one, 0, 0, _totalWidth, _totalHeight);

                /* Re-apply current scale factor. */
                if (autoScale)
                {
                    int scaleFactor = (int)SMUtil.DPIScaleFactor(true);
        ...
        private void CreateChildren()
		{
			...
			{
				SMUtil.AddToUILayer(t1.gameObject);
				foreach (Transform t2 in t1.transform)
				{
					SMUtil.AddToUILayer(t2.gameObject);
				}
			}
        ...
        static ArrayList fpsHistory = new ArrayList();
		...
		private void UpdateData(bool forceUpdate = false, float timeElapsed = -1.0f)
		{
			...
			if (_style == SMStyle.Minimal) return;
            ...
            fpsHistory.Add(fpsAvg);

            /* Calculate fixed update rate. */
            fixedUpdateRate = 1.0f / Time.fixedDeltaTime;

			/* Calculate MEM values. */
			memTotal = UnityEngine.Profiling.Profiler.GetTotalReservedMemory() / MEMORY_DIVIDER;
			memAlloc = UnityEngine.Profiling.Profiler.GetTotalAllocatedMemory() / MEMORY_DIVIDER;
		...
		private void Update()
		{
			...
			if (Input.anyKeyDown)
			{
				...
				else if (_mode == SMMode.Active && (((modKeyAlignment != KeyCode.None && hotKeyAlignment != KeyCode.None) && (Input.GetKey(modKeyAlignment) && Input.GetKeyDown(hotKeyAlignment))) || ((modKeyAlignment == KeyCode.None && hotKeyAlignment != KeyCode.None) && Input.GetKeyDown(hotKeyAlignment))))
				{
					NextAlignment();
				}
				/* Check for _style key combinations. */
				else if (_mode == SMMode.Active && (((modKeyStyle != KeyCode.None && hotKeyStyle != KeyCode.None) && (Input.GetKey(modKeyStyle) && Input.GetKeyDown(hotKeyStyle))) || ((modKeyStyle == KeyCode.None && hotKeyStyle != KeyCode.None) && Input.GetKeyDown(hotKeyStyle))))
				{
					NextStyle();
				}
			}
			...
			else if (switchStyleTapCount > 0 && (_mode == SMMode.Active && Input.touchCount == 2))
			{
			...
			else if (switchAlignmentTapCount > 0 && (_mode == SMMode.Active && Input.touchCount == 1))
			{
		...
		{
			if (!_isInitialized || _mode == SMMode.Inactive) return;
			...
			if (_mode == SMMode.Active) CreateChildren();
        ...
        static public string GetFPSHistory()
        {
            string fpsStr = "";
            for (int i = 0; i < fpsHistory.Count; ++i)
            {
                fpsStr = ((int)fpsHistory[i]) + ", ";
            }
            return fpsStr;
        }
