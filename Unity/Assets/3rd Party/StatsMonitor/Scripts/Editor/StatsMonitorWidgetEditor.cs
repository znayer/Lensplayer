...
// 
// StatsMonitorWidgetEditor.cs - version 1.0.0 - Updated 01-02-2016 17:42
//  
...
namespace StatsMonitor
{
	...
	internal sealed class StatsMonitorWidgetEditor : Editor
	{
		// ----------------------------------------------------------------------------
		// Properties
		// ----------------------------------------------------------------------------
		...
		public override void OnInspectorGUI()
		{
			...
			/* General parameters section */
			if (StatsMonitorEditorUtil.PropertyChanged(_mode, new GUIContent("Mode",
				...
				_self.Mode = (SMMode)_mode.enumValueIndex;
			if (StatsMonitorEditorUtil.PropertyChanged(_renderMode, new GUIContent("Render Mode",
				...
				_self.RenderMode = (SMRenderMode)_renderMode.enumValueIndex;
			if (StatsMonitorEditorUtil.PropertyChanged(_style, new GUIContent("Style",
				...
				_self.Style = (SMStyle)_style.enumValueIndex;
			if (StatsMonitorEditorUtil.PropertyChanged(_theme, new GUIContent("Theme",
				"Provides a selection of preset themes that affect the background color transparency and text/graph effects.")))
			{
				_self.Theme = (SMTheme)_theme.enumValueIndex;
			...
			if (StatsMonitorEditorUtil.PropertyChanged(_alignment, new GUIContent("Alignment",
				"Determines the position of the stats monitor.")))
				_self.Alignment = (SMAlignment)_alignment.enumValueIndex;
			if (StatsMonitorEditorUtil.PropertyChanged(_keepAlive, new GUIContent("Keep Alive", "If checked prevents the stats monitor object from being destroyed on level (scene) load.")))
				_self.KeepAlive = _keepAlive.boolValue;
			if (StatsMonitorEditorUtil.PropertyChanged(_statsUpdateInterval, new GUIContent("Stats Update Interval", "The time, in seconds at which the text displays are updated.")))
				_self.StatsUpdateInterval = _statsUpdateInterval.floatValue;
			if (StatsMonitorEditorUtil.PropertyChanged(_graphUpdateInterval, new GUIContent("Graph Update Interval", "The time, in seconds at which the graph is updated.")))
				_self.GraphUpdateInterval = _graphUpdateInterval.floatValue;
			if (StatsMonitorEditorUtil.PropertyChanged(_objectsCountInterval, new GUIContent("Object Count Interval", "The time, in seconds at which objects are counted.")))
				_self.ObjectsCountInterval = _objectsCountInterval.floatValue;

			EditorGUILayout.Space();

			if (StatsMonitorEditorUtil.PropertyChanged(_inputEnabled, new GUIContent("Input Enabled", "You can choose to disable Stats Monitor's own hotkey- and touch input polling and instead control the API externally, for example with another input manager.")))
				_self.inputEnabled = _inputEnabled.boolValue;

			/* Hot keys section */
			if (StatsMonitorEditorUtil.Foldout(_hotKeyGroupToggle, "Hot Keys"))
			{
			...
			if (StatsMonitorEditorUtil.Foldout(_touchControlGroupToggle, "Touch Control"))
			{
				EditorGUI.indentLevel = 1;
			...
			/* Look and feel section */
			if (StatsMonitorEditorUtil.Foldout(_layoutAndStylingToggle, "Layout & Styling"))
			{
				EditorGUI.indentLevel = 1;
				if (StatsMonitorEditorUtil.PropertyChanged(_fontFace, new GUIContent("Font", "The font used for all text fields. Leave blank to use the included font.")))
					_self.FontFace = (Font)_fontFace.objectReferenceValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_fontSizeLarge, new GUIContent("Large font size", "Font size for the FPS counter.")))
					_self.FontSizeLarge = _fontSizeLarge.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_fontSizeSmall, new GUIContent("Small font size", "Font size for all small text.")))
					_self.FontSizeSmall = _fontSizeSmall.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_padding, new GUIContent("Padding", "The padding between the outer edges and text fields.")))
					_self.Padding = _padding.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_spacing, new GUIContent("Spacing", "The spacing between text fields.")))
					_self.Spacing = _spacing.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_graphHeight, new GUIContent("Graph height", "Determines the height of the graph.")))
					_self.GraphHeight = _graphHeight.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_autoScale,
				...
				if (StatsMonitorEditorUtil.PropertyChanged(_scale, new GUIContent("Scale", "Allows to scale up the size of the stats monitor view by a multiplier.")))
					_self.Scale = _scale.intValue;
				GUI.enabled = true;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorBGUpper, new GUIContent("BG Color Top", "The color and transparency of the background panel top.")))
					_self.ColorBgUpper = _colorBGUpper.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorBGLower, new GUIContent("BG Color Bottom", "The color and transparency of the background panel bottom.")))
					_self.ColorBgLower = _colorBGLower.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorGraphBG, new GUIContent("Graph Background Color", "The background color and transparency of the graph area.")))
					_self.ColorGraphBG = _colorGraphBG.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFPS, new GUIContent("FPS Color", "Color used for the FPS value.")))
					_self.ColorFPS = _colorFPS.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFPSWarning, new GUIContent("FPS Warning Color", "The FPS counter will use this color if FPS falls below the FPS warning threshold.")))
					_self.ColorFPSWarning = _colorFPSWarning.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFPSCritical, new GUIContent("FPS Critical Color", "The FPS counter will use this color if FPS falls below the FPS critical threshold.")))
					_self.ColorFPSCritical = _colorFPSCritical.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFPSMin, new GUIContent("FPS Min Color", "Color used for the FPS minimum value.")))
					_self.ColorFPSMin = _colorFPSMin.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFPSMax, new GUIContent("FPS Max Color", "Color used for the FPS maximum value.")))
					_self.ColorFPSMax = _colorFPSMax.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFPSAvg, new GUIContent("FPS Average Color", "Color used for FPS average value.")))
					_self.ColorFPSAvg = _colorFPSAvg.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorMS, new GUIContent("Milliseconds Color", "Color used for the milliseconds value.")))
					_self.ColorMS = _colorMS.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorGCBlip, new GUIContent("GC Blip Color", "Color used for garbage collection graph blip.")))
					_self.ColorGCBlip = _colorGCBlip.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorFXD, new GUIContent("Fixed Update Color", "Color used for Fixed Update 'frame'-rate.")))
					_self.ColorFxd = _colorFXD.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorObjCount, new GUIContent("Objects Count Color", "Color used for the 'currently rendered objects' / 'total render objects' / 'total game objects' values.")))
					_self.ColorObjectCount = _colorObjCount.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorMemTotal, new GUIContent("Total Memory Color", "Color used for the total memory value.")))
					_self.ColorMemTotal = _colorMemTotal.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorMemAlloc, new GUIContent("Allocated Memory Color", "Color used for the allocated memory value.")))
					_self.ColorMemAlloc = _colorMemAlloc.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorMemMono, new GUIContent("Mono Memory Color", "Color used for the mono memory value.")))
					_self.ColorMemMono = _colorMemMono.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorSysInfoOdd, new GUIContent("System Info Odd Color", "Color used for odd sysinfo rows.")))
					_self.ColorSysInfoOdd = _colorSysInfoOdd.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorSysInfoEven, new GUIContent("System Info Even Color", "Color used for even sysinfo rows.")))
					_self.ColorSysInfoEven = _colorSysInfoEven.colorValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_colorOutline, new GUIContent("Outline Color", "Color used for text and graph outline. Setting alpha to 0 will remove the effect components.")))
			...
			/* FPS-Specific section */
			if (StatsMonitorEditorUtil.Foldout(_fpsGroupToggle, "FPS Specific"))
			{
				EditorGUI.indentLevel = 1;
				EditorGUILayout.BeginHorizontal(GUILayout.ExpandWidth(true));
				if (StatsMonitorEditorUtil.PropertyChanged(_throttleFrameRate, new GUIContent("Throttle FPS", "Enables or disables FPS throttling. This allows to see how the game performs under a specific frame rate. Does not guarantee selected frame rate. Set this to -1 to render as fast as possible in current conditions.\n\nIMPORTANT: this option disables VSync while enabled!")))
					_self.ThrottleFrameRate = _throttleFrameRate.boolValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_throttledFrameRate, new GUIContent()))
					_self.ThrottledFrameRate = _throttledFrameRate.intValue;
				EditorGUILayout.EndHorizontal();
				if (StatsMonitorEditorUtil.PropertyChanged(_avgSamples, new GUIContent("Average FPS Samples", "The amount of samples collected to calculate the average FPS value from. Setting this to 0 will result in an average FPS value calculated from all samples since startup or level load.")))
					_self.AverageSamples = _avgSamples.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_warningThreshold, new GUIContent("FPS Warning Threshold", "The threshold below which the FPS will be marked with warning color.")))
					_self.WarningThreshold = _warningThreshold.intValue;
				if (StatsMonitorEditorUtil.PropertyChanged(_criticalThreshold, new GUIContent("FPS Critical Threshold", "The threshold below which the FPS will be marked with critical color.")))
